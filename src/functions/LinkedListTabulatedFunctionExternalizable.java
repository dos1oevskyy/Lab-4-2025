...
public class LinkedListTabulatedFunctionExternalizable implements TabulatedFunction, Externalizable {
    ...
    public LinkedListTabulatedFunctionExternalizable(double leftX, double rightX, int pointsCount) {
        if (leftX >= rightX) {
    ...
    public LinkedListTabulatedFunctionExternalizable(double leftX, double rightX, double[] values) {
        if (leftX >= rightX) {
            throw new IllegalArgumentException("Левая граница должна быть меньше правой границы: " + leftX + " >= " + rightX);
        }
        if (values.length < 2) {
